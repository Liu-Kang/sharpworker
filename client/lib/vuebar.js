...
;(function(){
    ...
    Vuebar.install = function(Vue, options){
        ...
        /**
         * set scroll position
         * @param {[DOM]} el  [scrollbox dom]
         * @param {[number/top/bottom]} pos [top value]
         * @date 2017/07/25 liukang
         */
        function setPosition(el, pos){
            var state = getState(el)
            if (typeof pos === 'number') {
                state.el2.scrollTop = pos
            }
            if (pos === 'top') {
                state.el2.scrollTop = 0
            }
            if (pos === 'bottom') {
                state.el2.scrollTop = state.el2.scrollHeight - state.el2.clientHeight
            }
        }
